package safecar.yiye.apackage.com.safecar.MVP.Update.download;
...
public class DownloadApk {
    ...
    public static void downloadApk(Context context, String url, String title, final String appName) {
        ...
        //获取存储的下载ID
        long downloadId = MyApplication.getInstance().getLong(DownloadManager.EXTRA_DOWNLOAD_ID,-1L);
        if(downloadId != -1) {
            //存在downloadId
    ...
    private static void start(Context context, String url, String title, String appName) {
        ...
        if(hasSDKCard()) {
            ...
            MyApplication.getInstance().setLong(DownloadManager.EXTRA_DOWNLOAD_ID,id);
        } else {
    ...
    public static void removeFile(Context context) {
        String filePath = MyApplication.getInstance().getString("downloadApk",null);
        if(null != filePath) {
